...
class OrderTrackingApiIntegrationTest : PostgresIntegrationTestSupport() {
    ...
    fun `stores requested status event when order request is created`() {
        ...
        mockMvc
            .perform(get("/api/v1/strategies/${prepared.strategyId}/orders/requests/${prepared.orderRequestId}/status-events?limit=50"))
    ...
    fun `records partial and full fills and projects current position`() {
        ...
        mockMvc
            .perform(get("/api/v1/strategies/${signal.strategyId}/orders/requests?limit=20"))
            .andExpect(status().isOk)
        ...
        mockMvc
            .perform(get("/api/v1/strategies/${signal.strategyId}/orders/requests/$orderRequestId/status-events?limit=50"))
            .andExpect(status().isOk)
        ...
        mockMvc
            .perform(get("/api/v1/strategies/${signal.strategyId}/fills?limit=50"))
            .andExpect(status().isOk)
        ...
        mockMvc
            .perform(get("/api/v1/strategies/${signal.strategyId}/positions"))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$", hasSize<Any>(1)))
