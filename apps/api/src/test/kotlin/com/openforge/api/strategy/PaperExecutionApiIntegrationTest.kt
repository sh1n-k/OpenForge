...
class PaperExecutionApiIntegrationTest : PostgresIntegrationTestSupport() {
    ...
    fun `processes due schedule once and stores signals`() {
        ...
        mockMvc.perform(get("/api/v1/strategies/$strategyId/execution/runs?limit=20"))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$", hasSize<Any>(1)))
            .andExpect(jsonPath("$[0].status").value("completed"))
            .andExpect(jsonPath("$[0].signalCount").value(1))

        mockMvc.perform(get("/api/v1/strategies/$strategyId/signals?limit=50"))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$", hasSize<Any>(1)))
            .andExpect(jsonPath("$[0].symbol").value("AAA"))
            .andExpect(jsonPath("$[0].signalType").value("entry"))

        mockMvc.perform(get("/api/v1/strategies/$strategyId/execution"))
            .andExpect(status().isOk)
    ...
    private fun noSignalPayload(name: String) = mapOf(
        "builderState" to mapOf(
            "metadata" to mapOf(
                "id" to name.lowercase().replace(" ", "_"),
                "name" to name,
