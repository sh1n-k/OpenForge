              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-1 w-full">
                    <span className="text-[0.6875rem] font-bold tracking-wider uppercase bg-border-soft text-slate-600 px-1.5 py-0.5 rounded-md flex-shrink-0">{command.group}</span>
                    <span className={`font-semibold text-[0.9375rem] truncate flex-1 ${active ? "text-primary" : "text-foreground"}`}>{command.label}</span>
                  </div>
                  <span className={`text-[0.8125rem] truncate w-full pl-0.5 ${active ? "text-blue-600/80" : "text-muted"}`}>{command.description}</span>
                </button>
              );
            })
          )}
